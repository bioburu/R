...
GSE243609-Medulloblastoma tumorigenesis.
samples: cerebellum of Ptch1- Atoh1EGFP mice at P7, 6 weeks, and adulthood.
...
#DimPlot(rna, reduction = "umap", label = TRUE, pt.size = 0.1) #+ NoLegend()
#--start atac here
...
#head(Idents(atac))
#head(Idents(rna))
#DimPlot(object = atac, label = TRUE,pt.size = 0.1)
rna_umap <- DimPlot(rna, label = TRUE,pt.size = 0.1,label.size = 2)+ ggtitle("gene_expression")+ NoLegend()
#atac_features<-FeaturePlot(atac,features = c('Sox2','Olig2','Mycn','Mrc1','Rbfox3',
#                                             'Mki67','Gli2'),cols = c('grey','red'),pt.size = 1)
#atac_features
atac_umap<-DimPlot(object = atac, label = TRUE,pt.size = 0.1,label.size = 2)+ ggtitle("chromatin_accessibility")+ NoLegend()
rna_umap
atac_umap
#-------------------------------------------------------------------
#peaks <- FindMarkers(
...
Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) rstudio/2024.04.2+764 Chrome/120.0.6099.291 Electron/28.3.1 Safari/537.36, Quarto 1.4.555
